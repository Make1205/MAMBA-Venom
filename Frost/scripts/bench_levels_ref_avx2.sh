...
backend_tag(){
  ...
  if [[ "$mode" == "REFERENCE" ]]; then
    ...
    if [[ "${FORCE_USE_AVX2_FOR_L256:-0}" == "1" ]]; then
      echo "avx2_u16_forced"
    else
      echo "fast_u16_no_avx2"
    fi
  else
...
audit_backend(){
  ...
  if [[ "$mode" == "REFERENCE" ]]; then
    ...
    if [[ "$force" == "1" ]]; then
      use_avx2=1
      effective="frost_macrify.c forced USE_AVX2 u16 path"
    else
      use_avx2=0
      effective="frost_macrify.c FAST/RWCF u16 path (USE_AVX2 undefined in level file)"
    fi
  else
    use_avx2=1
    use_avx2_u32=1
