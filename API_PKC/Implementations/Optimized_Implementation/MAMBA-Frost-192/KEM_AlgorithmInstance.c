...
#define CRYPTO_SECRETKEYBYTES   14736ULL
#define CRYPTO_PUBLICKEYBYTES   11744ULL
#define CRYPTO_BYTES              24ULL
#define CRYPTO_CIPHERTEXTBYTES  11792ULL
...
/* Generate MAMBA-Frost-192 keypair and output lengths. */
int kem_keygen(
    ...
    unsigned char *sk, unsigned long long *sk_len_bytes)
{
    ...
    *sk_len_bytes = kem_get_sk_len_bytes();
    ...
    if (crypto_kem_keypair_Venom192(pk, sk) != 0) {
...
/* Encapsulate with MAMBA-Frost-192 public key and output lengths. */
int kem_enc(
    unsigned char *pk, unsigned long long pk_len_bytes,
    ...
    unsigned char *ct, unsigned long long *ct_len_bytes)
{
    ...
    *ct_len_bytes = kem_get_ct_len_bytes();
    ...
    if (crypto_kem_enc_Venom192(ct, ss, pk) != 0) {
        return VENOM_KEM_INTERNAL_ERROR;
    }
...
/* Decapsulate MAMBA-Frost-192 ciphertext and output shared-secret length. */
int kem_dec(
    unsigned char *sk, unsigned long long sk_len_bytes,
    unsigned char *ct, unsigned long long ct_len_bytes,
    unsigned char *ss, unsigned long long *ss_len_bytes)
{
    ...
    *ss_len_bytes = kem_get_ss_len_bytes();
    ret = crypto_kem_dec_Venom192(ss, ct, sk);
    if (ret == 0) {
        return VENOM_KEM_SUCCESS;
    }
