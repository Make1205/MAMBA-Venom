...
#define CRYPTO_SECRETKEYBYTES  11432ULL
#define CRYPTO_PUBLICKEYBYTES  9624ULL
#define CRYPTO_BYTES              24ULL
#define CRYPTO_CIPHERTEXTBYTES 9688ULL
    ...
    unsigned char *sk, unsigned long long *sk_len_bytes)
{
    ...
    *sk_len_bytes = kem_get_sk_len_bytes();
    ...
    if (crypto_kem_keypair_Frost192(pk, sk) != 0) {
    ...
    unsigned char *ct, unsigned long long *ct_len_bytes)
{
    ...
    *ct_len_bytes = kem_get_ct_len_bytes();
    ...
    if (crypto_kem_enc_Frost192(ct, ss, pk) != 0) {
        return FROST_KEM_INTERNAL_ERROR;
    }
    ...
    unsigned char *ss, unsigned long long *ss_len_bytes)
{
    ...
    *ss_len_bytes = kem_get_ss_len_bytes();
    ret = crypto_kem_dec_Frost192(ss, ct, sk);
    if (ret == 0) {
        return FROST_KEM_SUCCESS;
    }
