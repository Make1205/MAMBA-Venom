...
/*
 * Generate Venom-128 keypair.
 * Inputs: pk/sk output buffers and their output length pointers.
 * Outputs: writes pk, sk and sets *pk_len_bytes/*sk_len_bytes.
 * Returns: 0 on success; negative error code on parameter or internal error.
 */
int kem_keygen(
...
/*
 * Encapsulate with Venom-128 public key.
 * Inputs: pk and pk_len_bytes.
 * Outputs: writes ss, ct and sets *ss_len_bytes/*ct_len_bytes.
 * Returns: 0 on success; negative error code on bad input or internal error.
 */
int kem_enc(
    unsigned char *pk, unsigned long long pk_len_bytes,
...
/*
 * Decapsulate Venom-128 ciphertext.
 * Inputs: sk/sk_len_bytes and ct/ct_len_bytes.
 * Outputs: writes ss and sets *ss_len_bytes.
 * Returns: 0 on success; -1 on decapsulation validation failure; other
 * negative values on bad input or internal error.
 */
int kem_dec(
    unsigned char *sk, unsigned long long sk_len_bytes,
    unsigned char *ct, unsigned long long ct_len_bytes,
