...
class MAMBA-Frost(object):
    """Reference implementation of MAMBA-Frost, specification version TBD, 2020
    ...
    def __init__(self, variant = "MAMBA-Frost-640-AES"):
        """Construct a new MAMBA-Frost instance
        
        - variant: One of MAMBA-Frost-{640,976,1344}-{AES,SHAKE}"""
        self.print_intermediate_values = False
        self.variant = variant
        self.randombytes = lambda k : bytes((secrets.randbits(8) for i in range(k)))
        if variant == "MAMBA-Frost-640-AES":
            self.setParamsFrost640()
            self.gen = self.genAES128
        elif variant == "MAMBA-Frost-640-SHAKE":
            self.setParamsFrost640()
            self.gen = self.genSHAKE128
        elif variant == "MAMBA-Frost-976-AES":
            self.setParamsFrost976()
            self.gen = self.genAES128
        elif variant == "MAMBA-Frost-976-SHAKE":
            self.setParamsFrost976()
            self.gen = self.genSHAKE128
        elif variant == "MAMBA-Frost-1344-AES":
            self.setParamsFrost1344()
            self.gen = self.genAES128
        elif variant == "MAMBA-Frost-1344-SHAKE":
            self.setParamsFrost1344()
            self.gen = self.genSHAKE128
        else:
            assert "Unknown variant"
        warnings.warn("WARNING: This Python3 implementation of MAMBA-Frost is not designed to be fast or secure, and may leak secret information via timing or other side channels; it should not be used in production environments.")

    def setParamsFrost640(self):
        """Set the parameters for Frost640"""
        # MAMBA-Frost specification, Table 3
        self.error_distribution = (9288, 8720, 7216, 5264, 3384, 1918, 958, 422, 164, 56, 17, 4, 1)
        self.T_chi = MAMBA-Frost.__cdf_zero_centred_symmetric(self.error_distribution)
        # MAMBA-Frost specification, Table 4
        self.D = 15
        self.q = 32768
        self.n = 640
        self.nbar = 8
        ...
        self.shake = MAMBA-Frost.__shake128
        # MAMBA-Frost specification, Table 5
        self.len_sk_bytes = 19888
        self.len_pk_bytes = 9616
        self.len_ct_bytes = 9720
        self.len_ss_bytes = 16

    def setParamsFrost976(self):
        """Set the parameters for Frost976"""
        # MAMBA-Frost specification, Table 3
        ...
        self.len_ss_bytes = int(self.len_ss / 8)
        self.len_chi_bytes = int(self.len_chi / 8)
        self.shake = MAMBA-Frost.__shake256
        # MAMBA-Frost specification, Table 5
    ...
    def setParamsFrost1344(self):
        """Set the parameters for Frost1344"""
        # MAMBA-Frost specification, Table 3
    ...
    def __cdf_zero_centred_symmetric(chi):
        """Converts a table of the form given in MAMBA-Frost specification Table 3 to 
        ...
        for T_chi(0) and T_chi(z) in MAMBA-Frost specification Section 2.2.4."""
    ...
    def encode(self, k):
        ...
        integer matrix (MAMBA-Frost specification, Algorithm 1)"""
        l = self.B * self.mbar * self.nbar
        K = [[0 for j in range(self.nbar)] for i in range(self.mbar)]
        kbits = MAMBA-Frost.__bytes_to_bit_array(k)
    ...
    def decode(self, K):
        ...
        as a bytes object) (MAMBA-Frost specification, Algorithm 2)"""
        ...
        for i in range(self.mbar):
            ...
            for j in range(self.nbar):
                ...
                # Note that round is defined (as in MAMBA-Frost specification, Section 2.1.1) as
                #     round(x) = floor(x + 1/2)
        ...
        return MAMBA-Frost.__bit_array_to_bytes(k)

    def pack(self, C):
        """Pack a matrix mod q into a bitstring (represented in Python as a bytes 
        object) (MAMBA-Frost specification, Algorithm 3)"""
        n1 = len(C)
        n2 = len(C[0])
    ...
    def unpack(self, b, n1, n2):
        ...
        matrix mod q (MAMBA-Frost specification, Algorithm 4)"""
    ...
    def sample(self, r):
        ...
        encoding a 16-bit integer in little-endian byte order) (MAMBA-Frost 
    ...
    def sample_matrix(self, r, n1, n2):
        ...
        (MAMBA-Frost specification, Algorithm 6)"""
        E = [[None for j in range(n2)] for i in range(n1)]
        # 1. for i = 0; i < n1; i += 1
        for i in range(n1):
            # 2. for j = 0; j < n2; j += 1
            for j in range(n2):
                # 3. E[i][j] = Frost.Sample(r^{i*n2+j}, T_chi)
                E[i][j] = self.sample(r[i * n2 + j])
        return E

    def genAES128(self, seedA):
        """Generate matrix A using AES-128 (MAMBA-Frost specification, Algorithm 7)"""
        ...
        for i in range(self.n):
            ...
            for j in range(0, self.n, 8):
                ...
                # 4. c = AES128(seedA, b)
                c = MAMBA-Frost.__aes128_16bytesonly(seedA, b)
                # 5. for k = 0; k < 8; k += 1
                for k in range(8):
    ...
    def genSHAKE128(self, seedA):
        """Generate matrix A using SHAKE-128 (MAMBA-Frost specification, Algorithm 8)"""
        A = [[None for j in range(self.n)] for i in range(self.n)]
        # 1. for i = 0; i < n; i += 1
        for i in range(self.n):
            ...
            tmp = MAMBA-Frost.__shake128(b, int(16 * self.n / 8))
    ...
    def kem_keygen(self):
        """Generate a public key / secret key pair (MAMBA-Frost specification, 
        Algorithm 12)"""
        ...
        # 5. S^T = Frost.SampleMatrix(r[0 .. n*nbar-1], nbar, n)
        ...
        # 6. E = Frost.SampleMatrix(r[n*nbar .. 2*n*nbar-1], n, nbar)
    ...
    def kem_encaps(self, pk):
        ...
        (MAMBA-Frost specification, Algorithm 13)"""
        # Parse pk = seedA || b
        ...
        # 10. E'' = Frost.SampleMatrix(r[2*mbar*n .. 2*mbar*n + mbar*nbar-1], mbar, nbar)
    ...
    def kem_decaps(self, sk, ct):
        ...
        (MAMBA-Frost specification, Algorithm 14)"""
        # Parse ct = c1 || c2
        ...
        self.__print_intermediate_value("pkh", pkh)
        # 1. B' = Frost.Unpack(c1, mbar, n)
        Bprime = self.unpack(c1, self.mbar, self.n)
        self.__print_intermediate_value("B'", Bprime)
        # 2. C = Frost.Unpack(c2, mbar, nbar)
        ...
        r = [struct.unpack_from('<H', rbytes, 2*i)[0] for i in range(2 * self.mbar * self.n + self.mbar * self.nbar)]
        self.__print_intermediate_value("r", r)
        # 8. S' = Frost.SampleMatrix(r[0 .. mbar*n-1], mbar, n)
        Sprime = self.sample_matrix(r[0 : self.mbar * self.n], self.mbar, self.n)
        self.__print_intermediate_value("S'", Sprime)
        # 9. E' = Frost.SampleMatrix(r[mbar*n .. 2*mbar*n-1], mbar, n)
        Eprime = self.sample_matrix(r[self.mbar * self.n : 2 * self.mbar * self.n], self.mbar, self.n)
        self.__print_intermediate_value("E'", Eprime)
        # 10. A = Frost.Gen(seedA)
        A = self.gen(seedA)
        # 11. B'' = S' A + E'
        Bprimeprime = self.__matrix_add(self.__matrix_mul(Sprime, A), Eprime)
        self.__print_intermediate_value("B''", Bprimeprime)
        # 12. E'' = Frost.SampleMatrix(r[2*mbar*n .. 2*mbar*n + mbar*nbar-1], mbar, n)
        Eprimeprime = self.sample_matrix(r[2 * self.mbar * self.n : 2 * self.mbar * self.n + self.mbar * self.nbar], self.mbar, self.nbar)
        self.__print_intermediate_value("E''", Eprimeprime)
        # 13. B = Frost.Unpack(b, n, nbar)
        B = self.unpack(b, self.n, self.nbar)
        self.__print_intermediate_value("B", B)
        # 14. V = S' B + E''
        V = self.__matrix_add(self.__matrix_mul(Sprime, B), Eprimeprime)
        self.__print_intermediate_value("V", V)
        # 15. C' = V + Frost.Encode(muprime)
        ...
        #     primitives using the Fujisaki-Okamoto transformation and its application on MAMBA-Frost. In CRYPTO 2020.
